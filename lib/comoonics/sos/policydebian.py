...
class DebianSosPolicy:
    ...
    def allPkgs(self, ds = None, value = ""):
        import commands
        # if possible return the cached values
        try:                   return self._cache_pkg[ "%s-%s" % (ds,value) ]
        except AttributeError: self._cache_pkg = {}
        except KeyError:       pass

        _packages=commands.getoutput("dpkg-query -W -f='${Package} %s\n'" %value).split("\n")
        for _package in _packages:
            self._cache_pkg[ "%s-%s" % (ds,value) ] = [pkg for pkg in _packages]
        del mi, ts
        return self._cache_pkg[ "%s-%s" % (ds,value) ]
