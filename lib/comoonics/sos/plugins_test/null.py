...
class null(sos.plugintools.PluginBase):
    """This is an null plugin for sos. Plugins gather, analyze, and report on various aspects
    ...
    optionList = [("initd",  'Gathers the init.d directory', 'slow', False),
                  ('follicles', 'Gathers information about each follicle on every toe', 'slow', False),
                  ('color', 'Gathers toenail polish color', 'fast', False),
                  ("mtab", "Just copies the mtab", "fast", False) ]
    ...
    def setup(self):
        ...
        if self.isOptionEnabled("initd"):
            self.addCopySpec("/etc/init.d") # copies a whole directory tree
            
        if self.isOptionEnabled("mtab"):
            self.addCopySpec("/etc/mtab")
    ...
    def analyze(self):
        ...
        and use them here
        '''
        # This is an null of opening and reading the output of a command that
