...
FCPATH_HOST_TARGET=FCPATH_HOST_DEVICE+"/rport-%u:%u-%u/target%u:%u:%u"
...
SCSITARGET_PATTERN="rport-(\d+):(\d+)-(\d+)"
   ...
   if not host:
      ...
      if os.path.isfile(SCSISCAN_PATH %(host)) and os.access(SCSISCAN_PATH %(host), os.W_OK):
         ...
         for device_file in os.listdir(remove_dir):
            ...
            if match:
               scsi_host, scsi_bus, scsi_id=match.groups()
               for lun_file in os.listdir(SCSIPATH_2_DEVICE):
